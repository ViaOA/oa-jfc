...
public class OAComboBox extends JComboBox implements OATableComponent, OAJfcComponent {
	...
	public void setPopupColumns(int cols) {
		...
		if (listenPopupMenu == null) {
			this.removePopupMenuListener(listenPopupMenu);
		}
			...
			public @Override void popupMenuWillBecomeVisible(PopupMenuEvent e) {
				...
				if (exht == 0) {
					...
					if (comp instanceof JComponent) {
						exht += getExtraHeight((JComponent) comp);
					}
				}
				d.height += exht;
				pop.setSize(d);
	...
	*/
	...
	public void requestFocus() {
		...
		if (focusComp != this) {
			...
			if (focusComp instanceof OATextField) {
				((OATextField) focusComp).selectAll();
			}
		} else {
			super.requestFocus();
		}
		...
		for (int i = start;; i++) {
			...
			if (obj instanceof String) {
				...
				if (obj instanceof OAObject) {
					s = control.getValueAsString(obj);
				}
			}
	...
	/**
	 * Used to supply the renderer for the rows/objects in the list. Called by
	 * ComboBoxController.MyListCellRenderer.getListCellRendererComponent to get renderer.
	 */
