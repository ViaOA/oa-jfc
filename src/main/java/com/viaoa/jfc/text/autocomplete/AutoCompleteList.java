...
public abstract class AutoCompleteList extends AutoCompleteBase {
	...
	public AutoCompleteList(JTextField txt, final JList list, boolean bExactMatchOnly) {
		...
		if (border == null) {
			border = BorderFactory.createEmptyBorder(1, 5, 1, 16);
		} else {
			border = new CompoundBorder(BorderFactory.createEmptyBorder(1, 5, 1, 16), border); // extra is needed
		}
	...
	protected void onMouseOver(int row, MouseEvent evt) {
		...
		if (rectMouseOver != null) {
			list.repaint(rectMouseOver);
		}
		if (row < 0) {
			rectMouseOver = null;
		} else {
			rectMouseOver = list.getCellBounds(row, row);
		}
	}
	...
	protected Dimension updateSelectionList(String text, int offset) {
		...
		Dimension d;
		try {
			d = list.getPreferredScrollableViewportSize();
		} catch (Exception e) {
	...
	protected boolean onSelection() {
		...
		if (pos < 0 && list.getModel().getSize() == 1) {
			pos = 0;
		}
	...
	/**
	 * This can be used to auto-fill the remainder of value. Example: if user enters 'abc' and there is a match for 'abcdef', the 'def' can
	 * also be filled in. Returning value should be the least that is done.
	 */
	...
	/**
	 * Main method to override. Called when an item from Jlist is [clicked], [enter] on, or textField [enter] and there is only one item in
	 * Jlist.
	 * 
