 ...
 *
 ...
 Note on using html Tables:
 column sizing:
 	using a style width will make it fixed width
 		but the data can "push" it wider
 		the column will not share extra space
 	using an html width % will allow it to grow
 		and divides space with other columns that are not fixed.

Example:
<th style="width: 20px; text-align: right">#</th>
<th style="width: 75px; text-align: left"><nobr>Created</nobr></th>
<th width="6%" style="text-align: left"><nobr>Name</nobr></th>
<th width="93%" style="text-align: left"><nobr>Code Directory</nobr></th>
...
public class OAHTMLTextPane extends JTextPane implements OAPrintable {
	 ...
	 *
	 * @param binDir ex: bin/com/scheduler
	...
	class MyTransferable implements Transferable {
		...
		public DataFlavor[] getTransferDataFlavors() {
			int i = 0;
			if (textDataFlavor != null) {
				i++;
			}
			if (!bUsePlain && htmlDataFlavor != null) {
				i++;
			}
			...
			i = 0;
			if (textDataFlavor != null) {
				dfs[i++] = textDataFlavor;
			}
			if (!bUsePlain && htmlDataFlavor != null) {
				dfs[i] = htmlDataFlavor;
			}
			return dfs;
		}
	...
	protected int doPrint(final Graphics graphics, final PageFormat pageFormat, final int pageIndex) {
		if (pageFormat == null || printBoxView == null) {
			return NO_SUCH_PAGE;
		}
		...
		if (!allocate(vecPage, printBoxView, printBoxView.getPageWidth(), pageHeight, pageIndex)) {
			return NO_SUCH_PAGE;
		}
		...
		if (pageIndex > 0) {
			ppPrev = (PrintPage) vecPage.elementAt(pageIndex - 1);
		}
	...
	/**
	 * @param viewStart         the View that will be at the top of the page
	 * @param bIgnorePageBreaks true if this is already in a pageBreak
	 * @return
	 */
	protected boolean allocateViews(View view, Rectangle rect, PrintPage pp, int pageHeight, View viewStart, boolean bIgnorePageBreaks) {
		...
		for (int i = 0; i < x; i++) {
			...
			if ((rectChild.y + rectChild.height) <= pp.top) {
				continue;
			}

			// 2: the start for this view is after the bottom
			if (rectChild.y > pp.bottom) {
				continue;
			}

			View viewChild = view.getView(i);
			if (!bFoundStartView) {
				bFoundStartView = viewStart == viewChild;
			}
			...
			boolean bNoBr = bFoundStartView
					&& ((viewChild.getElement().getAttributes().getAttribute("nobr") != null) || (viewChild.getViewCount() == 0));
			...
			if (bJumbo || (rectChild.y >= pp.top && ((rectChild.y + (rectChild.height - 1)) <= pp.bottom))) {
				...
				if (bNoBr) {
					if (rectChild.y >= pp.top && pp.bottom >= rectChild.y) {
						pp.bottom = (rectChild.y - 1);
					}
			...
			bFoundStartView = allocateViews(viewChild, rectChild, pp, pageHeight,
											(bFoundStartView && viewChild != viewStart) ? null : viewStart, bIgnorePageBreaks);
			if (pp.viewPageBreak != null) {
				break;
			}
			if (holdBottom != pp.bottom) {
				break; // bottom has changed need to
						// rerun
			}
		}
	...
	protected int paintViews(Graphics g, View view, Rectangle rect, int top, int bottom, int pageHeight) {
		...
		for (int i = 0; i < x; i++) {
			Rectangle rectChild = (Rectangle) view.getChildAllocation(i, rect);
			if (rectChild == null) {
				continue; // no width and no height
			}

			// 1: view on previous page
			if ((rectChild.y + rectChild.height) <= top) {
				continue;
			}

			// 2: the start for this view is after the bottom
			if (rectChild.y > bottom) {
				continue;
			}
	 ...
	 *
	 * @param clazz   Class used for classLoaded.getResourceAsStream
	 * @param dirPath path to used to find image, use "/" as separator
	 */
	public void setImageLoader(Class clazz, String dirPath) {
		...
		if (pathForImages != null && !pathForImages.endsWith("/")) {
			pathForImages += "/";
		}
	}
	...
	/**
	 * insert a div that has an background image, and the div is sized to fit the image. This can then be used to place data over the image,
	 * like a certficate.
	 */
	...
	/**
	 * Counter used to track how many ties updateImage has been called, to know if there has been any image changes.
	 */
	...
	/**
	 * This is called from the MyImageView to get an image associated with an image tag. It will first use the image cache, and if not found
	 * will call loadImage.
	 *
	...
	/**
	 * Called by getImage. This will call imageHandler first. This method can be used as a hook, so that an application can retrieve images
	 * that are used in a document.
	 */
	...
	/**
	 * Base/root class to use when inserting Field tags. Set to non-OAObject class to to have it disabled.
	 */
	...
	/**
	 * Get the class to use for the list of Fields that a user can insert into a document. If fieldClass was not set by calling
	 * setFieldClass(..), then it will look for a method named "get"+propertyPath+"FieldClass" from the current Hub's class. If not found
	 * then it will use the Hub's class.
	 *
	 * @return
	 */
	...
	/**
	 * removed, to "not use" the enabledController, need to call it directly - since it has 2 params now, and will need to be turned on and
	 * off
	 *
	 * @Override public void setEnabled(boolean b) { if (getController() != null && getController().getBindController() != null) {
	 *           getController( ).getBindController().getEnabledController().directlySet(true, b); } super.setEnabled(b); }
	 */
	/**
