...
public class OAHTMLTextPaneController extends OATextController {
	...
	protected void updateEnabled() {
		...
		if (menuBar != null) {
			setEnabled(menuBar, b);
		}
		if (toolBar != null) {
			setEnabled(toolBar, b);
		}
		if (popupMenu != null) {
			setEnabled(popupMenu, b);
		}
		...
		htmlEditor.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if (e.getClickCount() != 2) {
					if (!bIsControlKeyDown || (!bIsMouseOverLink && !bIsMouseOverImage)) {
						return;
					}
				}
	...
	protected void updateCursorForLinkOrImage() {
		...
		if (bIsMouseOverImage) {
			if (bIsControlKeyDown) {
				s = "click the mouse to edit this image";
			} else {
				s = "<html>to edit this image: hold down the [Ctrl] key<br> and click the mouse, or double click the mouse";
			}
			if (htmlEditor == null || !htmlEditor.isEnabled() || !htmlEditor.isEditable()) {
				s = null;
			}
		} else if (bIsMouseOverLink) {
			if (bIsControlKeyDown) {
				s = "click the mouse to select this link";
			} else {
				s = "<html>to select this link: hold down the [Ctrl] key<br> and click the mouse, or double click the mouse";
			}
		} else {
			s = null;
		}
	...
	*****************************/
	...
	public JComboBox getFontNameComboBox() {
		if (cboFontName == null) {
			...
			for (int i = 0; i < genericFontFamily.length; i++) {
				vec.add(genericFontFamily[i]);
			}
	...
	*****************************/
	...
	public ComboBoxEditor getFontSizeComboBoxEditor() {
		if (fontSizeComboBoxEditor != null) {
		...
		ml = new MenuListener() {
			public void menuSelected(MenuEvent e) {
				if (htmlEditor != null) {
					...
					if (menuBackgroundColor != null) {
						menuBackgroundColor.setColor(c);
					}

					c = StyleConstants.getForeground(attr);
					if (menuFontColor != null) {
						menuFontColor.setColor(c);
					}
				}
			}
		...
		ml = new MenuListener() {
			public void menuSelected(MenuEvent e) {
				if (htmlEditor != null) {
					...
					Color c = StyleConstants.getBackground(attr);
					if (pmenuBackgroundColor != null) {
						pmenuBackgroundColor.setColor(c);
					}

					c = StyleConstants.getForeground(attr);
					if (pmenuFontColor != null) {
						pmenuFontColor.setColor(c);
					}
				}
			}
	...
	*/
	...
	protected void onInsertImage() {
		...
		if (file.length() > getMaxImageSize()) {
			JOptionPane.showMessageDialog(	getWindow(), "Image file over " + getMaxImageSize() + ", please use smaller size", "Open image",
	...
	protected String getInsertImageDivHtmlForEditorKitCallback() {
		...
		if (file.length() > getMaxImageSize()) {
			JOptionPane.showMessageDialog(	getWindow(),
											"Image file size greater then max limit of " + getMaxImageSize() + ", please use smaller size",
											"Open image", JOptionPane.WARNING_MESSAGE);
			return null;
		}
		...
		try {
			...
			System.out.println("Exception: " + ex);
			ex.printStackTrace();
			JOptionPane.showMessageDialog(	getWindow(), "Error reading image \n" + srcName, "OAHTMLTextPaneController",
											JOptionPane.WARNING_MESSAGE);
			return null;
		}
	...
	/**
	 * Gets Attributes from the current View. The View's attributes are needed since they have the CSS styles in the attributes
	 */
	protected View getCurrentView() {
		int pos = htmlEditor.getSelectionStart();
		if (pos == 0) {
			pos = 1; // 0 = first paragraph
		}
		if (pos < 0) {
			return null;
		}
	...
	public void setSelection(int xStart, int xFinish, boolean moveUp) {
		if (htmlEditor != null) {
			if (moveUp) {
				...
				htmlEditor.select(xStart, xFinish);
			}
		}
	}

	/**
	 * find the first occurrence of an <code>Element</code> in the element tree above a given <code>Element</code>
	 *
	...
	/**
	 * find the first occurrence of an <code>Element</code> in the element tree below a given <code>Element</code>
	 *
	 * @param name   the name of the <code>Element</code> to search for
