...
public class AutoCompleteTextFieldController extends OAJfcController {
	...
	public void setSortResults(boolean b) {
		this.bSortResults = b;
	}
	...
	public void update() {
		try {
			if (autoCompleteList != null) {
				autoCompleteList.bIgnorePopup = true;
			}
			_update();
		} finally {
			if (autoCompleteList != null) {
				autoCompleteList.bIgnorePopup = false;
			}
		}
		super.update();
	}
	...
	public OATemplate getTemplateForSearch() {
		if (OAString.isNotEmpty(getSearchTemplate())) {
			if (templateSearch == null) {
				templateSearch = new OATemplate<>(getSearchTemplate());
			}
		}
	...
	public AutoCompleteList getAutoCompleteList() {
		...
		autoCompleteList = new AutoCompleteList(this.txt, jlist, true) { // true=exact match only
			...
			private void _search(Hub h, String text, final int cnt) {
				for (Object obj : h) {
					...
					if (obj instanceof OAObject) {
						if (getTemplateForSearch() != null) {
							searchValue = getTemplateForSearch().process((OAObject) obj);
						} else if (getPropertyPath() != null) {
							searchValue = ((OAObject) obj).getPropertyAsString(getPropertyPath());
						} else {
			...
			protected String[] getSearchData(String text, int offset) {
				...
				if (alList.size() >= maxResults) {
					ss[x] = "<html><i>Limited to " + x + " max results</i>";
				}
				return ss;
			}
			...
			protected String getToolTipText(int pos) {
				...
				if (OAString.isEmpty(tsi.toolTip)) {
					if (getTemplateForToolTipText() == null) {
						return tsi.displayValue;
					}
					if (tsi.obj instanceof OAObject) {
						tsi.toolTip = getTemplateForToolTipText().process((OAObject) tsi.obj);
					}
					if (tsi.toolTip != null && tsi.toolTip.toLowerCase().indexOf("html") < 0) {
						tsi.toolTip = "<html>" + tsi.toolTip;
					}
				}
				return tsi.toolTip;
			}
