...
public class ToggleButtonController extends OAJfcController implements ItemListener {
	...
	/**
	 * Bind a button to a property path to the active object for a Hub. Button wil be enabled based on both active object and propertyPath
	 * in Hub not being null.
	 */
	...
	/**
	 * Bind a button to a property path to the active object for a Hub. Button will be enabled based on active object in Hub not being null.
	 * param valueOn value to use for property when button is selected
	 */
	...
	/**
	 * Bind a button to a property path to the active object for a Hub. Button wil be enabled based on active object in Hub not being null.
	 * 
	 * @param valueOn  value to use for property when button is selected
	 * @param valueOff value to use for property when button is not selected
	 */
	...
	/**
	 * Second Hub that is used to determine if this button is enabled. Button is enabled based on the active object not being null.
	 */
	...
	/**
	 * The value to use if the property is null. Useful when using primitive types that might be set to null.
	 * <p>
	 * Example:<br>
	 * A boolean property can be true,false or null. Might want to have a null value treated as false.<br>
	 * setNullValue(false);
	 */
	...
	public @Override void afterChangeActiveObject() {
		if (DEBUG) {
			System.out.println("aaaaaaaaaaa " + (++aa) + ", bFlag=" + bFlag);
		}
		try {
			if (bFlag) {
				return;
	...
	protected void _afterChangeActiveObject() {
		...
		if (hubSelect != null) {
			...
			if (obj == null && valueOn == null) {
				b = true;
			} else if (obj == null || valueOff == null) {
				b = false;
			} else {
				if (valueOff == null) {
					b = true; // 2006/10/26 if off is set to null, then any non-null is true
				} else {
					b = obj.equals(valueOn);
				}
			}
			if (xorValue > 0) {
				int x = 0;
	...
	protected boolean _itemStateChanged(ItemEvent evt) {
		...
		if ((hubSelect != null) || hub != null) {
			...
			if (obj != null) {
				...
				if (hubSelect != null) {
					if (type == ItemEvent.SELECTED) {
						if (hubSelect.getObject(obj) == null) {
							hubSelect.add(obj);
						}
					} else {
					...
					try {
						...
						if (msg != null) {
							...
							bValid = false;
						} else if (!confirmPropertyChange(obj, value)) {
							bValid = false;
						}
						...
						boolean wasChanged = ((OAObject) obj).getChanged();
						setValue(obj, value);
						/* was
						method = getSetMethod();
