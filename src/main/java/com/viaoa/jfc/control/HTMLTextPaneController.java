...
public abstract class HTMLTextPaneController extends OAJfcController implements FocusListener {
	...
	public Class getFieldClass() {
		if (fieldClass != null) {
			if (OAObject.class.isAssignableFrom(fieldClass)) {
				return fieldClass;
			}
		...
		try {
			...
			if (bValueChangedWhileEditing || bChange) {
				...
				if (bChange) {
					...
					if (!confirmPropertyChange(activeObject, newText)) {
						return false;
					}
				}
				...
				if (bChange) {
					...
					if (getEnableUndo()) {
						OAUndoManager.add(OAUndoableEdit.createUndoablePropertyChange(	undoDescription, activeObject, endPropertyName, hold,
																						newText, wasChanged));
					}
	...
	/**
	 * This is called before the text is saved to a property, in cases where the data was changed by another user while this user was
	 * editing. This is not called if the user did not make any changes from the original value. Note: if a property is being edited
	 * (hasFocus), then propertyChanges are ignored.
	 * 
