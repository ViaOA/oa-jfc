...
public class OAJfcController extends HubListenerAdapter {
	...
	public Object getValue(Object obj) {
		...
		if (bIsHubCalc) {
			obj = getOA().services().objects().reflect().getProperty(getHub(), propertyPath);
	...
	*/
	...
	/**
	 * The "word(s)" to use for the empty slot (null value). Example: "none of the above". Default: "" (blank). Set to null if none should
	 * be used
	 */
	...
	protected void callUpdate() {
		...
		if (!SwingUtilities.isEventDispatchThread()) {
			if (!abUpdate.compareAndSet(false, true)) {
				return;
			}
