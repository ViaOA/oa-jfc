...
public class OATableColumn {
	...
	public void setFormat(String fmt) {
		if (oaComp instanceof OAJfcComponent) {
			((OAJfcComponent) oaComp).getController().setFormat(fmt);
		}
		this.format = fmt;
	}
	...
	/**
	 * 20180620 get the last OAObject in the property path, for case where a column uses a property path instead of just a property.
	 * 20181004 changed to get object from table.hub in the component.hub
	 */
	...
	public OAObject getColumnObject(OAObject objStart, final Class endClass) {
		...
		Method[] ms = methods;
		if (ms == null || ms.length < 2) {
			if (oaObj.getClass().equals(endClass)) {
				return oaObj;
			}
			return null;
		}
	...
	public Method[] _getMethods(Hub hubTable) {
		if (methods != null && hubTable == hubMethodHub) {
			return methods;
		}
