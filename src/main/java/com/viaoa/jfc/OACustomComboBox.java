...
public abstract class OACustomComboBox extends JComboBox implements OATableComponent, OAJfcComponent {
	...
	/**
	 * Directly called by popup to set property value. This can be overwritten to provide custom features for setting property.
	 */
	...
	public void setSelectedItem(Object item) {
		...
		try {
			...
			if (control != null) {
				control.updatePropertyValue(item);
			}
	...
	/**
	 * Property used for image name, that is used to get image to display with rows.
	 */
	...
	*/
	...
	/**
	 * Used to determine the default width, based on average character width of font.
	 */
	...
	public void setMaximumColumns(int x) {
		...
		if (vtf != null) {
			...
			if (c > 0) {
				vtf.setMaximumColumns(c);
			} else {
				c = vtf.getMaximumColumns();
				if (c > 0) {
					this.setMaximumColumns(c);
				}
			}
		}
		invalidate();
	...
	public void setMinimumColumns(int x) {
		...
		if (vtf != null) {
			...
			if (c > 0) {
				vtf.setMinimumColumns(c);
			} else {
				c = vtf.getMinimumColumns();
				if (c > 0) {
					this.setMinimumColumns(c);
				}
			}
		}
		invalidate();
	...
	*/
	...
	protected JComponent getFocusComponent() {
		...
		if (getEditor() != null) {
			...
			if (!(focusComp instanceof OATextField)) {
				focusComp = null; // dont use default editor
			}
		}
		...
		if (focusComp == null) {
			...
			for (int i = 0; i < comps.length; i++) {
				if (comps[i] instanceof JComponent) {
					...
					if (focusComp.isRequestFocusEnabled()) {
						break;
					}
					focusComp = null;
				}
