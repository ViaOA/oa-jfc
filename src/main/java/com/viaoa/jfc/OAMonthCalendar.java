...
/**
 * UI for displaying items in a Calendar. Given a hub and date properties to use, this will then display them by day. If the date property
 * is unique (only one) then another property (many) can be used for this display of objects for that day, and an optional create command
 * can be added for days that dont have an object that exists. If date is not unique, then they will be combined by day.
 */
public class OAMonthCalendar<F extends OAObject, T extends OAObject> extends JScrollPane {
	...
	/**
	 * Called when a new month is being displayed. This is used when working with objectCache, so that objects can be loaded from datasource
	 * that match the selected date range for the new month.
	 *
	 * @see #getBeginDate()
	 * @see #getEndDate()
	 */
	...
	protected void setSelectedDate(OADate dx, final boolean bFromDayPanel) {
		...
		if (bNewMonth) {
			...
			for (F obj : hub) {
				for (String pp : datePropertyPaths) {
					Object objx = obj.getProperty(pp);
					if (!(objx instanceof OADateTime)) {
						continue;
					}
					dx = new OADate((OADateTime) objx);
		...
		for (DayPanel dp : alDayPanel) {
			...
			if (dp.hub.getSize() == 0) {
				dp.lbl.setText(dp.date.toString());
			}
	...
	protected void setupHubs() {
		...
		hub.addHubListener(new HubListenerAdapter<F>() {
			...
			public void afterChangeActiveObject(HubEvent<F> e) {
				...
				Object objx = obj.getProperty(datePropertyPaths[0]);
				if (!(objx instanceof OADateTime)) {
					return;
				}
				OADate dx = new OADate((OADateTime) objx);
			...
			public void afterPropertyChange(HubEvent<F> e) {
				if (!propNamex.equalsIgnoreCase(e.getPropertyName())) {
					return;
				}
				remove(e.getObject());
				add(e.getObject());
	...
	public void setToolTipTextTemplate(String s) {
		...
		for (DayPanel dp : alDayPanel) {
			if (dp.lbl instanceof OALabel) {
				((OALabel) dp.lbl).setToolTipTextTemplate(toolTipTextTemplate);
			}
		}
	}
