...
public class OAJfcUtil {
	...
	*/
	...
	public static void blink(JComponent component, Color fgColor, Color bgColor, int numberOfTimes, final int msDelay) {
		...
		if (fgColor == null) {
			fgColor = component.getBackground();
		}
		if (bgColor == null) {
			bgColor = component.getForeground();
		}
		...
		synchronized (hmBlinker) {
			...
			if (blinkerx == null) {
				...
				blinkerx.bOpaque = component.isOpaque();
			} else {
				bFound = true;
			}
		...
		ActionListener al = new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (b) {
					component.setOpaque(blinker.bOpaque);
					synchronized (hmBlinker) {
	...
	public static String setLookAndFeel(String laf) throws Exception {
		...
		for (String id : al) {
			...
			if (!(objx instanceof ColorUIResource)) {
				continue;
			}
			if (objx.equals(c)) {
