...
public class OATreeNode implements Cloneable {
	...
	/**
	 * @param hub
	 *            is either the Hub or UpdateHub, depending if this is a top level node.
	 */
	...
	// 20120302
	/**
	 * Need to be able to have the root Hub for recursive hub not change. public void useRecursiveRootHub() { if (hub == null) return; Hub
	 * hubRoot = hub.getRootHub(); if (hubRoot == null) return; Class clazz = hub.getObjectClass(); OAObjectInfo oi =
	 * OAObjectInfoDelegate.getOAObjectInfo(clazz); OALinkInfo li = oi.getRecursiveLinkInfo(OALinkInfo.MANY); if (li == null) return; Hub
	 * hubMaster = hubRoot.getMasterHub(); if (hubMaster == null) return; this.hub = new Hub(this.hub.getObjectClass()); // create a copy of
	 * the original hub // if you use a sharedHub, then setting the AO to an object in a child hub will set the shared hub to that new child
	 * hub final String prop = HubDetailDelegate.getPropertyFromMasterToDetail(hubRoot); OAObject obj = (OAObject) hubMaster.getAO(); if
	 * (obj != null) { Hub h = (Hub) obj.getProperty(prop); hubCopy = new HubCopy(h, this.hub, false); } hubMaster.addHubListener(new
	 * HubListenerAdapter() { Override public void afterChangeActiveObject(HubEvent e) { OATreeNode.this.hub.clear(); if (hubCopy != null)
	 * hubCopy.close(); hubCopy = null; Hub hubRoot = OATreeNode.this.hub.getRootHub(); if (hubRoot == null) return; Hub hubMaster =
	 * hubRoot.getMasterHub(); if (hubMaster != null) return; OAObject obj = (OAObject) hubMaster.getAO(); if (obj == null) return; Hub h =
	 * (Hub) obj.getProperty(prop); hubCopy = new HubCopy(h, OATreeNode.this.hub, false); } }); } private HubCopy hubCopy ;
	 */
	...
	/**
	 * This can be overwritten to provide a HubFilter for the Hub that will be used for each instanceof of this OATreeNode. <code>
	 * ex:
	...
	/**
	 * 2007/01/05 number of characters to use.
	 */
	...
	/**
	 * @param hub
	 *            is either Hub to get data from or updateHub.
	 */
	...
	/**
	 * @param updateHub
	 *            is the Hub to call setShared on.
	 */
	...
	public void setEditor(OATableComponent component) {
		...
		if (def.editor == null || !(def.editor instanceof Component)) {
			return;
		}
	...
	*/
	/**
	 * Path to find the image. Use "/", and include a leading "/" if it is from the beginning of the classpath, or root directory. Will use
	 * the classloader to also search jar files.
	 */
	...
	public void setTree(OATree tree) {
		...
		if (tree != null) {
			tree.root.setupTopHub(aiXcnt.incrementAndGet(), false);
		}
	}
	...
	*/
		...
		if (methodsToHub == null && bRecursive) {
			...
			if (method == null) {
				throw new RuntimeException(
						"OATreeNode.getHubForNodeData() cant find recursive method for " + clazz.getName() + "." + li.getName());
			}
		...
		for (pos = prev = 0; pos >= 0; prev = pos + 1) {
			...
			if (method == null) {
				if (OAObject.class.equals(clazz)) {
					...
					throw new RuntimeException("OATreeNode.getMethods() cant find method for \"" + name + "\" in PropertyPath \"" + fullPath
							+ "\", from class=" + clazz.getSimpleName());
				}
			}
	...
	/**
	 * method to override to allow an object of a different class to be dropped. By default, the classes have to be assignable.
	 */
	public boolean getAllowDrop(Hub hubDrag, Object objectDrag, Hub hubDrop) {
		...
		if (c == null || objectDrag.getClass().isAssignableFrom(c)) {
			return true;
		}
		return false;
	}
	...
	public void onDoubleClick(Object obj, MouseEvent e) {
		if (originalNode != null) {
			originalNode.onDoubleClick(obj, e);
		}
	}
	...
	public void objectSelected(Object obj) {
		if (originalNode != null) {
			originalNode.objectSelected(obj);
		}
	}
	...
	// was:
	/**
	 * This is called in the SwingWorker thread, to allow for background processing like getting data from server.
	 */
	/**
	...
	public void beforeObjectSelected(Object obj) {
		if (originalNode != null) {
			originalNode.beforeObjectSelected(obj);
		}
	}
	...
	public Component getTreeCellRendererComponent(Component comp, JTree tree, Object value, boolean selected, boolean expanded,
			boolean leaf, int row, boolean hasFocus, OATreeNodeData treeNodeData) {
	...
	public Component getTreeCellRendererComponent(Component comp, JTree tree, Object value, boolean selected, boolean expanded,
		...
		if (def.imageProperty != null) {
			if (obj != null) {
				if (def.methodsToImage == null) {
					def.methodsToImage = OAReflect.getMethods(obj.getClass(), def.imageProperty);
				}
		...
		if (color != null) {
			...
			if (color != null) {
				if (myColorIcon == null) {
					myColorIcon = new ColoredLineUnderIcon();
				}
		...
		if (def.toolTipTextProperty != null) {
			if (obj != null) {
				if (def.methodsToToolTipText == null) {
					def.methodsToToolTipText = OAReflect.getMethods(obj.getClass(), def.toolTipTextProperty);
				}
				if (def.methodsToToolTipText != null) {
		...
		if (ttt == null) {
			ttt = OAConv.toString(value);
		}
		ttt = getToolTipText(obj, ttt);
		if (ttt != null) {
			((JLabel) comp).setToolTipText(ttt);
		}
		...
		if (chkType != CHECK_NONE) {
			if (checkIcon == null) {
				checkIcon = new TNCheckIcon();
			}

			checkIcon.checkBox.setSelected(chkType == CHECK_FULL);
			checkIcon.drawHalfCheck = (chkType == CHECK_HALF);

			boolean bInCheckBox = (this.def.tree.rowLastMouse == row && this.def.tree.xLastMouse >= 0
					&& this.def.tree.xLastMouse < checkIcon.getIconWidth());
	...
	/**
	 * 2006/12/21 Called by renderer to get image name for a node. By default, this returns imagePath + defaultValue
	 * 
	 * @param imagePath
	 *            is path of image, which will end with a path seperator.
	 */
...
/**
 * replaced by jfc.image.ColorIcon class TNColorIcon implements Icon { Color color; public int getIconHeight() { return 17; } public int
 * getIconWidth() { return 12; } public void paintIcon(Component c,Graphics g,int x,int y) { g.setColor(color==null?Color.white:color);
